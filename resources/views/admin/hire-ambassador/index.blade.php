        ...
        <div class="col-xl-12">
          <div class="page-title-box">
            <div class="d-flex justify-content-between align-items-center text-muted">
              ...
              <i class="ti-crown"></i> Hired Ambassadors</h4>
        ...
        <div class="col-12">
            <div class="card-box table-responsive">
              <table id="responsive-datatable" class="table dt-responsive nowrap table-striped" cellspacing="0" width="100%">
                  ...
                  <tbody>
                    @foreach ($hire_ambassadors as $key=>$ambassador)
                    <tr>
                      <td>{{ $key + 1 }}</td>
                      <td>{{ $ambassador->name }}</td>
                      <td>{{ $ambassador->email }}</td>
                      <td>{{ $ambassador->phone }}</td>
                      <td>{{ $ambassador->ambassador_name }}</td>
                      <td>{{ $ambassador->created_at->diffForHumans() }}</td>
                      <td class="btn-group action-button">
                        <a href="{{ route('admin.hire-ambassador.show',$ambassador->id) }}" class="btn btn-info btn-sm">
                        ...
                        <form id="delete-form-{{ $ambassador->id }}" action="{{ route('admin.hire-ambassador.destroy', $ambassador->id) }}" method="POST" style="display: none;">
                          @csrf
                          @method('DELETE')
                        </form>
                      </td>
